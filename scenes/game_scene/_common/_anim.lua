...
function load_game_scene_anim_char_common_0_general_hurt_hard_recovery_wallstick_ground(obj_char)
    local res = {}
    local sprite_sheet_state = "0_general_hurt_hard_recovery_wallstick_ground"
    local side = obj_char["player_side"]
    local pushbox_data = common_game_scene_get_pushbox(side)
    local hurtbox_data = common_game_scene_get_hurtbox(side)
    local anchor_data = common_game_scene_get_anchor(side)
...
function load_game_scene_anim_char_common_0_general_hurt_soft_recovery_wallstick_air(obj_char)
    local res = {}
    local sprite_sheet_state = "0_general_hurt_soft_recovery_wallstick_air"
    local side = obj_char["player_side"]
    local pushbox_data = common_game_scene_get_pushbox(side)
    local hurtbox_data = common_game_scene_get_hurtbox(side)
    local anchor_data = common_game_scene_get_anchor(side)
...
-- burst_overdrive
    -- 需要修改的角色属性
        ...
        -- 对方
            ...
            -- game_speed_subframe
            -- game_speed_abnormal_realtime_countdown
function load_game_scene_anim_char_common_burst_overdrive(obj_char,other_side_countdown,special_init)
    local res = {}
...
-- 需要修改的角色属性
    ...
    -- 对方
        ...
        -- game_speed_subframe
        -- game_speed_abnormal_realtime_countdown
function load_game_scene_anim_char_common_burst_RC_red(obj_char,update_state)
    local res = {}
    local height_state = obj_char["height_state"]
    local obj_char_other_side = common_game_scene_change_character(obj_char["player_side"])
    ...
    for i = 0,44 do
        res[i] = function()
            -- state
            update_state(obj_char,obj_char_other_side)
        end
    end
    for i = 0,24 do
        res[i] = function()
...
function load_game_scene_anim_char_common_burst_RC_blue(obj_char,update_state)
    local res = {}
    local height_state = obj_char["height_state"]
    local obj_char_other_side = common_game_scene_change_character(obj_char["player_side"])
    local goal_heat_gauge_remain = math.max(0,obj_char["heat_gauge"][1]-100)
...
function load_game_scene_anim_char_common_burst_RC_purple(obj_char,update_state)
    local res = {}
    local height_state = obj_char["height_state"]
    local obj_char_other_side = common_game_scene_change_character(obj_char["player_side"])
    local goal_heat_gauge_remain = math.max(0,obj_char["heat_gauge"][1]-100)
...
function load_game_scene_anim_char_common_burst_RC_yellow(obj_char,update_state)
    local res = {}
    local height_state = obj_char["height_state"]
    local obj_char_other_side = common_game_scene_change_character(obj_char["player_side"])
    local goal_heat_gauge_remain = math.max(0,obj_char["heat_gauge"][1]-100)
    ...
    res[13] = function()
        ...
        -- game_speed
        -- 设置为强制速度到动画结束
        common_game_scene_game_speed_load_application(obj_char,{1,nil,nil,nil,nil,45-14})
        common_game_scene_game_speed_load_application(obj_char_other_side,{1,nil,nil,nil,45-14,nil})
        -- draw_correction
        obj_char[8] = 3
    end
    res[16] = function()
